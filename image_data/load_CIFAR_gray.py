...
def get_cifar_data_coe(index,name):
    ...
    #保存到coe文件
    file_name = open(name, 'w', encoding='utf-8')
    file_name.write('MEMORY_INITIALIZATION_RADIX = 10;\n')
    file_name.write('MEMORY_INITIALIZATION_VECTOR=\n')
    file_name.write('0,-4096,0,-4096,20480,-4096,0,-4096,0,\n')
    # file_name.close()
    # file_name = open(name, 'a', encoding='utf-8')
    np.savetxt(file_name, gray, fmt='%.0f')  #加上fmt='%f'能去掉科学计数法
    file_name.close()


    return gray

def get_cifar_data_txt(index,name):
    cifar10_dir = 'C:/Users/Michael/Desktop/Project/FPGA/image_data/CIFAR-10'
    X_rgb = load_cifar10(cifar10_dir)
